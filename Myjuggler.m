...
global JugFigH figNum JugFigTitle JugAxisH
...
elseif strcmp(action, 'initialize'),
    ...
    JugFigH = findall(0,'Name', JugFigTitle);
    JugCount = 1;
    if isempty(JugFigH)
...
elseif strcmp(action, 'set_all_gui'),
    ...
    JugFigH = figure( ...
        'Name',            JugFigTitle, ...
        'WindowStyle','normal', ...
        'CloseRequestFcn', 'juggler(0, 0, 0, 0, ''close'');', ...                             
        'NumberTitle',     'off', ...
        'DockControls',    'off', ...
        'Toolbar',         'none');
    % V4 default color
...
%elseif strcmp(action, 'set_extra_gui'),    % extra UI's
    ...
    cb1 = '';
    cb2 = [mfilename '(0,0,0,0,''set_mouse_action'')'];

    string1 = 'Controller:';
    string2 = 'Human|Fuzzy';

    [upH, upPos] = uiarray(Pos(1,:), 1, 2, spacing, 2*spacing, ...
        str2mat('text', 'popup'), ...
        str2mat(cb1, cb2), ...
        str2mat(string1, string2));
    set(upH(1), 'HorizontalAlignment', 'right');
    controllerH = upH(2);
    set(controllerH, 'value', 2);
    % ====== Appending handles as the second row of userdata
    tmp = [controllerH -1 -1 -1 -1 -1 -1 -1 -1 -1];
...
elseif strcmp(action, 'set_init_cond'),
    ...
    ud = get(findall(0, 'name', JugFigTitle), 'userdata');
    ud(3, 8) = x_prev;
    ud(3, 9) = y_prev;
    ud(3, 10) = v_prev;
    set(JugFigH, 'userdata', ud);
...
elseif strcmp(action, 'set_anim_obj'),
    ...
    set(ballH, 'color', 'c', 'clipping', 'off');
    ...
    set(plateH, 'color', 'y', 'linewidth', 5);
    ...
    set(arrowH, 'color', 'red');
    set(arrowH, 'clipping', 'off', 'userdata', arrow);
    set(arrowH, 'userdata', arrow);
    % ====== set 'userdata'
    ud = double([ballH plateH refH arrowH arrow_baseH]);
    ud = reshape(ud,[1 5]);
    ud = [ud -1 center -1 -1 -1];
...
elseif strcmp(action, 'load_fis_mat'),
    ...
    if ud(3, 6) < 0,    % FIS not been built
        JugFisMat = readfis('Myjuggler');
        ud(3, 6) = 1;
        set(JugFigH, 'userdata', ud);
    end

    name='Myjuggler';
    figNum=findall(0, 'name', ['Rule Viewer: ' name]);
    if isempty(figNum)
     ruleview(JugFisMat);
     figNum=findall(0, 'name', ['Rule Viewer: ' name]);
...
elseif strcmp(action, 'fuzzy_controller'), % fuzzy controller
    ...
    % ====== get animation objects
    ud = get(JugFigH, 'userdata');
    o1 = evalfis(JugFisMat,[x_hit - DesiredPos ProjAngle]);
    % ====== rule viewer
    xx=allchild(0);
    figN=[];
    for i=1:length(xx)
